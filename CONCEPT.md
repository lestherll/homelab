...
**Divergence narrowed by the Talos migration (2026-08-16).** The class side of that split now states the guarantee directly: `scratch` (disposable, Delete), `fast` and `bulk` (both durable, Retain, split by access pattern). More importantly the cluster sets **no default StorageClass**, so a volume whose durability was never considered no longer lands silently on whatever the distribution happened to make default — it stays `Pending`. That is not the unlabelled-volume alert this decision asks for, but it closes the same gap at the point of creation and needs no alert to do it, because the failure is loud by construction. The label side is unchanged and still not reconciled with the class side; `Application.persistence` joins `Database` and SeaweedFS in setting it. Which object is authoritative remains undecided.
...
**Decision.** Every build tags its image with the commit SHA (already happening) but the *deployment* pins by **digest**, never by a floating tag. The app's own build workflow — not a separate reconciler — writes the digest into its own `deploy/application.yaml` and pushes that commit to its own repo, using the `contents: write` permission it already effectively has by virtue of executing that repo's own code. It then **dispatches `register.yml`** (`workflow_dispatch`), which holds all the cluster access — tailnet join, OIDC token, `kubectl` — and force-reconciles the app's `GitRepository`/`Kustomization` via the `reconcile.fluxcd.io/requestedAt` annotation, so Flux picks the commit up in about a minute rather than on its 5-minute poll.

The dispatch is not indirection for its own sake: **a push made with the default `GITHUB_TOKEN` cannot trigger a workflow.** GitHub suppresses workflow runs from events raised with that token to prevent recursion, so `register.yml`'s `on: push` never sees the digest-pin commit. `workflow_dispatch` is a documented exception to that suppression, which is what makes an explicit call work where the implicit event does not. Discovered by shipping the implicit version first and watching it silently do nothing — recorded in `docs/journal/2026-08-26-github-token-cannot-trigger-workflows.md`.

Keeping the reconcile inside `register.yml` rather than copying it into each build workflow is the other half of the decision: cluster access stays in the one file `docs/zero-touch-app-registration.md` calls copyable-verbatim, and a build workflow stays a build — it knows the registry and its own repo, and nothing about Flux or the cluster.
...
**Rejected alternative: duplicating the tailnet-join/token-mint/annotate sequence into each build workflow**, so the build reconciles directly instead of dispatching. Rejected because it puts cluster access in two files per app repo that must then stay in sync, against a copyable-verbatim template — for a saving of roughly the twenty seconds a second workflow run takes to start. Its one real advantage is decoupling: a build could still reconcile while `register.yml` is broken.

**Cost:** each app's build workflow gains a commit-and-push step, a `paths-ignore: [deploy/**]` guard, an `actions: write` permission, and a one-line dispatch. `register.yml`'s copyable template (`docs/zero-touch-app-registration.md`) gains the reconcile step, still generic across every app repo. Delivery is now ~1 minute rather than instant, because the dispatched run has to start.

**Reverses if:** the number of app repos grows enough that per-repo build-workflow changes (rather than centralising in a reconciler) become the more expensive path — at which point the cross-repo-credential cost D19 rejected needs re-pricing against that duplication cost.
...
- **v0.11 (2026-08-26)** — D19's reconcile mechanism corrected. As first recorded it had `register.yml` firing on the digest-pin push; a push made with the default `GITHUB_TOKEN` cannot trigger a workflow, so that half never ran and delivery quietly fell back to Flux's 5-minute poll. The build workflow now dispatches `register.yml` explicitly (`workflow_dispatch` being a documented exception to the suppression rule). Decision unchanged in substance — who writes the bump, and pinning by digest, both stand; only the trigger moved. Journal: `docs/journal/2026-08-26-github-token-cannot-trigger-workflows.md`.
...
is built or decided.
**Written:** 2026-08-09, during review of the self-service platform API's
PoC plan (`~/.claude/plans/declarative-scribbling-yeti.md`).
...
or decided; a broker is deliberately *not* in that PoC's scope.

CONCEPT.md already anticipates this twice: its "should" list once named a
message broker on the same self-service pattern ("the specific list matters
less than the pattern generalising"), and one of its ground-path examples is
literally this scenario ("I add a few lines to a repo. It's provisioned,
credentials are delivered to my app, it's backed up, and it appears in
monitoring").

## Background: the conventions this builds on

From the self-service platform API's plan review rounds (the "attachment
pattern" these notes reuse):
...
- **Aliased attachments:** `databases: [{ ref: orders-db, as: main }]` →
  ...
  split, no implicit defaults (the layer-promotion rule: explicable, not magical).
...
- **Kind-approval gate:** a kind ships only if a proven operator can mint
  isolated, zero-touch credentials (the zero-touch-credentials pattern's
  success test), verified against
...
3. **It's an acceptance test the concept doc already wrote.** If the
   ...
   the zero-touch-credentials pattern's bet ("the pattern generalises") fails.
...
- **(a) Platform schema fields** (`queues: [...]` on the attachment) —
  ...
  leak the layer-promotion rule exists to prevent.
...
- **Strimzi (Kafka)** — strongest CRD fit in this space: `Kafka`,
  ...
  single-node KRaft, against the workload-headroom rule's "half the machine
  for workloads" on a 16GB node already running SeaweedFS + CNPG +
  observability.
...
## 5. Durability-classification nuance: message data is probably not "durable"
...
intent. Broker storage classification is a small, real amendment to the
durability-classification rule, to make at proposal time.
...
3. The kind gate gains a resource-budget clause (the workload-headroom rule
   applies to candidate engines).
4. The durability-classification rule gains the "message data is not
   durable" nuance (topology in git is the backup).

## Meta-observation

Every convention from the self-service platform API's PoC review —
name-is-the-handle, owning/attaching, prefix-scoped isolation,
contract-per-kind, additive schema evolution — transferred to a service the
plan never designed for, unchanged. That's what the pivot to that API's
thesis predicts ("cheap to revise in place") and is the strongest evidence
so far the API shape is right. The
